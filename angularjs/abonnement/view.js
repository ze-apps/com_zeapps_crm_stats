app.controller("ComZeappsStatsAbonnementCtrl", ["$scope", "$route", "$routeParams", "$location", "$rootScope", "zeHttp", "menu",
    function ($scope, $route, $routeParams, $location, $rootScope, zhttp, menu) {
        ...
        function loadList(context) {
            ...
            zhttp.quiltmania_stats.abonnement_info.get($scope.category.id, context, formatted_filters).then(function(response){
                if(response.data && response.data != "false"){
